...
public class CatalogDriver
{
    ...
    /**
     * This method uses Bubble-Sort to sort the specified List of CatalogItems.
     * @param <S> the ID type for each CatalogItem in catalogs over which CatalogItems are
     * ultimately sorted
    ...
    public static<S extends Media> void sort( List<CatalogItem<S>> catalog )
    {
        ...
        {
            j = i;
            while( j > 0 && catalog.get( j - 1 ).compareTo( catalog.get( j ) ) < 0 )
            {
